...
import com.example.focusapp.DAO.Dao;
import com.example.focusapp.UI.NotesAdapter;
...
public class Fragment_notes extends Fragment implements View.OnClickListener{
    ...
    @Override
    //这个函数在创建视图时调用，点击事件等方法不放在这里，这里用于加载控件（应该）
    ...
    @Override
    //这个在fragment所存在的活动oncreate方法执行完毕后被调用
    ...
    private void initAdapter (){
        ...
        adapter.setOnItemClickListener(new NotesAdapter.OnRecyclerViewItemClickListener() {
            @Override
            //弹出学习时间窗口，输入后跳转至计时器
            public void onClick(View view, int position, NotesAdapter.ViewName viewName) {
                Notes temp = mNotes.get(position);
                String noteContent = temp.getNotesContent();
                if (viewName.equals(NotesAdapter.ViewName.START)){
                    UpdatePos = position;
                    //传入被点击的note在list（mNotes）中的序号
                    dialoFragmentStartCount = DialoFragment_startCount.newIntance(noteContent);
