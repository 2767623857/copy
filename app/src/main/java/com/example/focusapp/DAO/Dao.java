package com.example.focusapp.DAO;
...
import com.example.focusapp.A_F.Notes;
...
//        }
    ...
    public double getTdCredits(int year, int month) {//获取当日积分
        SQLiteDatabase db = mHelper.getWritableDatabase();
        String sql = "Select * from " + Constants.TABLE_NAME +" where Years=" + year +
                " and Months=" + month + " Order By Days desc";
        Cursor cursor = db.rawQuery(sql, null);
        cursor.moveToNext();
        double Credits = cursor.getDouble(cursor.getColumnIndex("Credits"));
        //cursor.close();
